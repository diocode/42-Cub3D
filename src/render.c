...
/*   Updated: 2024/04/12 17:16:58 by digoncal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    set_dda(t_raycast *ray, t_data *data)
{
    ...
    {
        ray->step_x = -1;
        ray->horiz_x = (data->player->pos_x - ray->map_x) * ray->vert_x;
    }
    else
    {
        ray->step_x = 1;
        ray->horiz_x = (ray->map_x + 1.0 - data->player->pos_x) * ray->vert_x;
    }
    if (ray->dir_y < 0)
    {
        ray->step_y = -1;
        ray->horiz_y = (data->player->pos_y - ray->map_y) * ray->vert_y;
    }
    else
    {
        ray->step_y = 1;
        ray->horiz_y = (ray->map_y + 1.0 - data->player->pos_y) * ray->vert_y;
    }
}
...
void    ray_info_init(int x, t_raycast *ray, t_data *data)
{
    ...
    ray->dir_y = data->player->dir_y + data->player->plane_y * ray->ray_angle;
...
void    calculate_line_height(t_raycast *ray, t_data *data)
{
    ...
    if (ray->side == 0)
        ray->wall = data->player->pos_y + ray->distance * ray->dir_y;
    else
        ray->wall = data->player->pos_x + ray->distance * ray->dir_x;
